import { resolvePaths, mkDirSync } from './Util';
...
export default class {
    ...
    private assignFileValue(files: Files, fieldName: string, value: File) {
        ...
        if (isMultiValue) {
            ...
            target = isObject<FileCollection>(previous)? previous : {
                path: [], size: [], type: [], name: [], tmpName: []
            };
    ...
    private parseHeaders(headers: string[]): MultipartHeaders {
        ...
        const result: MultipartHeaders = {
            isFile: false, type: 'text/plain', fileName: '', fieldName: generateRandomText(8),
    ...
    private parseMultiPart(string: string, boundary: string | null) {
        ...
        string.split(boundaryLinePattern).slice(1, -1).forEach(bodyPart => {
