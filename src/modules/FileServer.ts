...
export default class FileServer {
    ...
    private endStream(filePath: string, response: Response, status: number, headers: Headers, options?: object) {
        ...
        return new Promise((resolve, reject) => {
            ...
            readStream.pipe((response as unknown) as NodeJS.WritableStream, {end: false});

        }).then(() => response.end()).catch(err => {
            readStream.close();
     ...
     */
    private validateRangeRequest(headers: IncomingHttpHeaders, eTag: string,
        fileMTime: string, fileSize: number) {

        const result: {isMultiRange: boolean, statusCode: number, ranges: Range[]} = {
            isMultiRange: false,
            statusCode: 206,
            ranges: []
        };
     ...
     * process file
     */
    private process(method: Method, filePath: string, request: Request, response: Response,
        status: number = 200, headers: Headers = {}) {
        ...
        if (isUndefined(request.headers['range'])) {
            ...
            if (ranges.length === 1) {
                ...
                return this.endStream(filePath, response, 206, resHeaders, {start, end});
            }
