...
describe('Router', function() {
    ...
    const getTemplate = (method: Method) => {
        return function() {
            const banner = 'should store the given route rule for ' +
            ...
            it(`can take a middleware callback or array of middleware callbacks as
                ...
                expect(router.getRoutes()[method][0][1]).toEqual(dummyCallback);
                expect(router.getRoutes()[method][0][2]).toEqual({middleware: [dummyMiddleware]});
            });
            ...
            or aray of middlewares which in turn is resolved to a ResolvedCallbackOptions object`, function() {
                ...
                router[method]('/users', dummyCallback, {middleware: dummyMiddleware});
                expect(router.getRoutes()[method].length).toEqual(1);
                expect(router.getRoutes()[method][0][0]).toEqual('users');
                expect(router.getRoutes()[method][0][1]).toEqual(dummyCallback);
                expect(router.getRoutes()[method][0][2]).toEqual({middleware: [dummyMiddleware]});
    ...
    describe(`#use(url: Url, middleware: Middleware | Middleware[],
        ...
        it(`can accept an array of http method arguments as options too`, function() {
            ...
            expect(router.getMiddlewares()[0][0]).toEqual('');
            expect(router.getMiddlewares()[0][1]).toEqual([dummyMiddleware]);
            expect(router.getMiddlewares()[0][2]).toEqual({method: ['get', 'post']});
        });
